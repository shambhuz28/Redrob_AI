...
QUERY_LIMIT = int(os.getenv("CANDIDATE_QUERY_LIMIT", "150"))
STRICT_HONEYPOT_FILTER = os.getenv("STRICT_HONEYPOT_FILTER", "false").lower() == "true"
HYDRATE_MISSING_SCORES = os.getenv("HYDRATE_MISSING_SCORES", "false").lower() == "true"
TOP_CANDIDATE_LIMIT = int(os.getenv("TOP_CANDIDATE_LIMIT", "100"))
...
def main() -> None:
    ...
    if HYDRATE_MISSING_SCORES:
        hydrate_missing_scores(client, candidate_scores, query_embeddings)
    else:
        print("Skipping missing vector hydration. Set HYDRATE_MISSING_SCORES=true to enable it.")
...
def build_qdrant_client() -> QdrantClient:
    ...
    if not url:
        raise ValueError("QDRANT_URL not found in environment or .env")
